...
class TaxonomySelectField(forms.ModelChoiceField):
  ...
  def __init__(self, level_indicator='-', *args, **kwargs):
    ...
    super(TaxonomySelectField, self).__init__(queryset=Taxonomy.objects.all())
...
class TermSelectField(forms.ModelChoiceField):
  ...
  def __init__(self, taxonomy_id, init_term_id, *args, **kwargs):
    #! personally, I think this is cobbled. Go to SQL join?
    term_ids = TermTaxonomy.objects.filter(taxonomy__exact=taxonomy_id).values_list('term', flat=True)
    #print('Term ids:')
    #print(str(term_ids))
    super().__init__(initial=init_term_id, queryset=Term.objects.filter(id__in=list(term_ids)))
...
class AllTermSelectField(forms.ModelChoiceField):
  ...
  def __init__(self, *args, **kwargs):
    #! personally, I think this is cobbled. Go to SQL join?
    #root_terms = TermTree.objects.filter(parent__null=True)
    #term_ids = TermTaxonomy.objects.filter(taxonomy__exact=taxonomy_id).values_list('term', flat=True)
    #print('Term ids:')
    #print(str(term_ids))
...
class TermForm(forms.ModelForm):
  ...
  parent = TermSelectField(taxonomy_id=1, init_term_id=3)
...
class TermAdmin(admin.ModelAdmin):
  ...
  def save_model(self, request, obj, form, change):
    ...
    #Select
    #if (not change):
    #TermTaxonomy(term=obj.id, taxonomy=obj.taxonomy)
    #model = TermTaxonomy(term=obj.id, taxonomy=obj.taxonomy)  
    #model.save()
    ...
    h = TermTree(term=obj.id, parent=obj.parent)  
    h.save()
...
class TermTreeAdmin(admin.ModelAdmin):
  # Not working
  #raw_id_fields = ("term",)
...
admin.site.register(TermTree, TermTreeAdmin)
...
class TermTaxonomyAdmin(admin.ModelAdmin):
  ...
  #raw_id_fields = ("term",)
  form = TermTaxonomyForm
  list_select_related = ('term', 'taxonomy')
